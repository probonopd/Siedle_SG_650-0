...
#define IMX21_WDOG_WMCR	0x08 /* Watchdog Miscellaneous Control Register */
...
static int imx_watchdog_restart(struct notifier_block *nb, unsigned long event, void *data)
{
	struct imx_wd *wd = container_of(nb, struct imx_wd, notify);
...
static int imx_wd_probe(struct device_d *dev)
{
	...
	priv->dev = dev;
	priv->notify.notifier_call = imx_watchdog_restart;
	...
	if (IS_ENABLED(CONFIG_WATCHDOG_IMX)) {
		...
		if (of_get_property(dev->device_node, "fsl,wdog-timeout",
				    &ret)) {
			/* Set WDT to enable WDOG_B time-out assertion */
			writew(IMX21_WDOG_WCR_WDT | IMX21_WDOG_WCR_SRS |
			       IMX21_WDOG_WCR_WDA, priv->base + IMX21_WDOG_WCR);
			/* Clear PDE to disable power down counter */
			writew(0, priv->base + IMX21_WDOG_WMCR);
		}
	}
