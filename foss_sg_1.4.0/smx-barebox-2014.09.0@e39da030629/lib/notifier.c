...
static NOTIFIER_HEAD(restart_notifier_list);

int register_restart_handler(struct notifier_block *nb)
{
	return notifier_chain_register(&restart_notifier_list, nb);
}

int unregister_restart_handler(struct notifier_block *nb)
{
	return notifier_chain_unregister(&restart_notifier_list, nb);
}

int restart_notifier_call_chain(unsigned long address)
{
	return notifier_call_chain(&restart_notifier_list, address, NULL);
}
